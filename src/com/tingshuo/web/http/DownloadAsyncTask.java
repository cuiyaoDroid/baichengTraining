...
public class DownloadAsyncTask extends AsyncTask<String, Integer, Boolean> {
	...
	protected Boolean doInBackground(String... params) {
		try {
			//String content =URLEncoder.encode(""+params[0], "UTF-8");
			
			HttpClient client = HttpsClient.getInstance().getHttpsClient();
			client.getParams().setParameter(
					CoreProtocolPNames.HTTP_CONTENT_CHARSET,
					Charset.forName("UTF-8"));
			String downloadUri=HttpJsonTool.ServerUrl+"/html/"+params[0].replaceAll(" ", "%20");
			HttpPost httpRequest = new HttpPost(downloadUri);
			Log.i("httpdownload",  HttpJsonTool.ServerUrl+"/html/"+params[0]);
			HttpResponse response;
			response = client.execute(httpRequest);
			HttpEntity entity = response.getEntity();

			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				downloadFile = new File(TrainingApp.appPath+FileNameTool.getExtensionName(params[0]));
